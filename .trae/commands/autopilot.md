...
13. 对 prompt/context harness 类任务，QA 必须覆盖 `context_mode=off|observe|inject`、`prompt_version=context_harness_v1`、request-only compaction、不泄露 raw prompt/context/image、以及 Safety/HITL 不变式；旧 text DSL prompt 回滚路径不得恢复。

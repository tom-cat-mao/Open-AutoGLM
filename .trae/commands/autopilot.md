---
description: 启动/管理 Autopilot：RALPLAN → execution → RALPH/verification → QA 持续执行流水线
argument-hint: "[status|cancel|resume|reset] [--use-current-plan|--direct] [--no-verification] [--no-qa] <task>"
---
...
2. 若未显式 `--use-current-plan` 或 `--direct`，默认先执行 RALPLAN。
...
若 hook 尚未注入 stage prompt，请按 Autopilot 协议从 `ralplan` stage 开始；若用户传入 `--use-current-plan` 且 `graph.mdc` 已 approved，可从 `execution` stage 开始。

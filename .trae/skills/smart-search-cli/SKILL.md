...
description: CLI-first web research and source retrieval through the local smart-search command. Use when TraeCLI needs current web search, source-backed fact checking, URL fetching, site mapping, official/API/documentation search, or reproducible search evidence via Skill + CLI instead of MCP tools.
---

# Smart Search CLI

Use the local `smart-search` command as the default execution layer for web research in TraeCLI. The skill decides routing; the CLI performs the work; JSON or saved files provide evidence.
...
4. If `doctor` returns `ok: true`, use only `smart-search` CLI subcommands for web research. Do not call TraeCLI native WebSearch in the same task.
5. This project vendors the skill under `.trae/skills/smart-search-cli`; do not refresh it into Codex/Claude user directories during normal TraeCLI work.
...
- If keys are changed with `smart-search config set`, rerun the CLI; no TraeCLI restart is needed.
- If PATH is changed, a new terminal or TraeCLI restart may be needed.
...
- In sandboxed runtimes (TraeCLI, containers, CI) where the default config directory is not writable or must be pinned, set `SMART_SEARCH_CONFIG_DIR` to an absolute writable path. The CLI uses it for both config and relative logs and skips default-directory selection.
...
# Project-local TraeCLI skill is vendored at .trae/skills/smart-search-cli.
# Do not run global skill update commands unless intentionally syncing upstream.

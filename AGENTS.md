...
| 17 | **Lesson Evolution (WP-L)** | `phone_agent/v2/evolution.py` is an offline proposal plane only. `--distill` reads privacy-minimal episode outcomes, accepts strict-JSON candidates only for evidence-qualified groups, charges `UsageLedger` role `distill`, and writes `lesson_proposed`; Rule-of-3 may mark a proposal eligible but only a human CLI action may approve/revoke/supersede it. `memory/lessons/events.jsonl` is truth and `lessons.json` is rebuildable. Lesson data must never enter `_initial_messages`, prompts, middleware, tools, routing, or any actor context; `PHONE_AGENT_MEMORY_RAG` stays shadow. |
...
| `phone_agent/v2/evolution.py` | Offline lesson proposals, append-only lifecycle events, Rule-of-3 evaluation, and human review primitives; zero actor injection. |

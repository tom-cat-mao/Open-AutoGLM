...
| ExpectedOutcome | `ExpectedOutcome` 是 ActionIR 的 sibling verification contract；Plan 可从 provider envelope 拆出 `action` 与 `expected_outcome`，但 `expected_outcome` 不得进入 canonical ActionIR、Validator/Safety/Executor，也不提供执行授权 |

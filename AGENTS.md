...
| Grounding | 主 VLM 的屏幕目标点击类动作只输出 `target_mark_id`；LocateAnything/Fake/OCR/UIAutomator/SoM 只能作为 MarkRegistry provider 生成 marks，`target_text_hint` 只能作为受控 provider hint，不能作为可执行目标；本地 provider 可在内存中使用 raw bounded hint，但 raw hint 不得进入 trace/checkpoint/prompt/report，远程 provider raw hint 必须显式 opt-in；provider 由 graph config/env 注入，不暴露给 tool schema；LocateAnything 默认输入图最长边 `max_size=960`，可用 `locateanything_max_size` / `PHONE_AGENT_LOCATEANYTHING_MAX_SIZE` 灰度或回滚 |

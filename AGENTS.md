...
| Grounding | 默认 `PHONE_AGENT_GROUNDING_PROVIDER=hybrid`；主 VLM 的屏幕目标点击类动作仍以 atomic `target_mark_id` 为唯一执行边界；`target_object_id`、`ordinal`、`object_role`、`object_filter` 只允许作为非执行 IntentIR selector，必须在 `phone_agent/actions/grounding.py` 用当前 `ObjectRegistry + MarkRegistry + ScreenBinding` 编译为唯一 `target_mark_id`；LocateAnything/Fake/OCR/UIAutomator/SoM 只能作为 MarkRegistry provider 生成 marks，`target_text_hint` 只能作为受控 provider hint，不能作为可执行目标；Android accessibility tree marks 来自 `adb exec-out uiautomator dump /dev/tty`，统一转换为 0-1000 bounds，并可生成 `structure_kind=accessibility` 的 `ScreenStructure`/`ObjectRegistry` sidecars；bounds parser 支持 signed/whitespace bounds，XML 控制字符只做清洗，未转义 ampersand/截断 hierarchy/结构错乱按 `accessibility_xml_parse_error` fail-closed；LocateAnything 结构模式默认 `off`，仅显式 `locateanything_structure_mode=target|screen` 时生成 `structure_kind=visual` sidecar，visual 对象必须带 source/tier/eligibility，resolver 必须硬校验 `executable_selector`，不能伪装 accessibility truth 或回退 raw bbox tap；hybrid 先用 accessibility tree，失败时才 fallback 到 LocateAnything，缺少 dump callback 或显式 skip tree child 时必须在 `hybrid_factory` 和 synthetic `fallback_chain` skip row 记录原因；不匹配当前 hint 的 provider marks 只能作为 diagnostic candidates/structures 保留，不得进入最终 executable `marks`；本地 provider 可在内存中使用 raw bounded hint，但 raw hint 不得进入 trace/checkpoint/prompt/report，远程 provider raw hint 必须显式 opt-in，默认只发送 redacted/bounded hint；remote_openai/stepfun/hybrid_remote 只能显式启用，API key 只经 env/config 注入且不暴露给 tool schema；新增 `parse_summary`、`fallback_chain`、`hybrid_factory`、`remote` 只允许 code/count/bool/hash/length/model/host，不得包含 raw XML/raw hint/raw OCR/private text/raw response/base64/API key，diagnostic code 字段必须使用 enum/safe-identifier allowlist；provider 由 graph config/env 注入，不暴露给 tool schema；LocateAnything/remote 默认输入图最长边 `max_size=960`，可用 `locateanything_max_size` / `PHONE_AGENT_LOCATEANYTHING_MAX_SIZE` 灰度或回滚；LocateAnything prompt 必须通过 `apply_chat_template(..., num_images=1)`，默认不注入额外 context，仅允许 bounded `locateanything_context_max_chars` |
...
| Grounding Benchmark | 正式入口在 `bench/grounding/`；post-training bbox 0-1 必须转换为 0-1000 manifest；LocateAnything benchmark 用 `.venv/bin/python -m bench.grounding.run_locateanything`，remote/StepFun benchmark 用 `.venv/bin/python -m bench.grounding.run_remote_provider`，固定 manifest 复评用 `.venv/bin/python -m bench.grounding.score_predictions`；真实 LocateAnything 依赖 Apple Silicon + Metal + mlx-vlm，remote benchmark 依赖显式 API key env，`bench_output/` 默认不提交 |
...
- Phase 完成后按项目规范更新 `.trae/rules/graph.mdc`，若改动架构/API/评测/trace/TraeCLI 编排，同步更新 `README.md`、`docs/future-roadmap.md`、`.trae/traecli.toml`、相关 `.trae/rules/*.mdc` 与本文件；commit message：`feat(graph): <phase 目标>`、`feat(trae): <phase 目标>` 或 `feat(grounding): <benchmark/grounding 目标>`。

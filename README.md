...
`PHONE_AGENT_MARKS_WINDOWED=auto|on|off`（默认 `auto`）控制窗口感知 marks（纯展示层）：`auto` 先试 `uiautomator dump --windows`，设备不支持则回退旧的单根 dump；`on` 强制 `--windows`（不支持则报错可见）；`off` 保持旧平铺渲染。模型看到的 marks 会按窗口分组、标注可操作性四档（`confirmed`/`likely`/`blocked`/`unknown`，启发式窗口不会给 `blocked`）和稀疏语义容器路径（`path=`，最多 3 层）。这是纯展示升级——mark id 徽章、`resolve_mark`、工具执行、安全门、折叠与 `locate` 一寸不动，`op=blocked` 本包只展示不拦截。

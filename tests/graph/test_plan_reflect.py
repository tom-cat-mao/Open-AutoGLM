...
def test_plan_node_uses_output_mode_prompt_when_no_override(
    base_state, fake_device
) -> None:
    base_state["messages"] = []
    base_state["step_count"] = 0
...
def test_plan_node_keeps_custom_system_prompt_with_output_mode(
    base_state, fake_device
) -> None:
    base_state["messages"] = []
    base_state["step_count"] = 0
...
def test_plan_node_wrong_page_parse_failure_falls_back_to_back(
    base_state, fake_device
) -> None:
    model = FakeModelClient(FakeModelResponse("think", "not an action"))
    ...
    assert (
        result["parse_metadata"]["deterministic_recovery_reason"]
        == "wrong_page_go_back"
    )
        ...
        def request(self, messages, **kwargs):
            ...
            if self.calls == 1:
                raise ModelParseError(
                    "raw invalid private text", {"parse_error_code": "invalid_json"}
                )
...
def test_plan_node_rejects_unknown_mark_intent(base_state, fake_device) -> None:
    model = FakeModelClient(
        FakeModelResponse(
            "", '{"type":"intent","action":"tap","target_mark_id":"missing"}'
        )
    )
...
def test_plan_node_stores_expected_outcome_as_sibling_contract(
    base_state, fake_device
) -> None:
    model = FakeModelClient(
        FakeModelResponse(
...
def test_plan_expected_outcome_runtime_contract_verifies_plain_text(
    base_state, fake_device
) -> None:
...
def test_plan_node_validates_structured_json_and_repairs_safe_action_alias(
    ...
    model = FakeModelClient(
        FakeModelResponse("", '{"type":"do","action":"wait","duration":1}')
    )
...
def test_plan_node_validator_rejects_dangerous_structured_field(
    ...
    model = FakeModelClient(
        FakeModelResponse(
            "",
            '{"type":"do","action":"Wait","duration":"1 seconds","command":"rm -rf /"}',
        )
    )
...
def test_plan_node_builds_provider_marks_before_model_and_uses_mark_id(
    base_state, fake_device
) -> None:
    ...
    assert (
        provider.requests[0]["screen_binding"]["raw_screenshot_hash"]
        == result["observation"]["snapshot"]["raw_screenshot_hash"]
    )
...
def test_plan_node_accessibility_marks_failure_does_not_abort(
    base_state, fake_device
) -> None:
...
def test_plan_node_hybrid_ignores_direct_accessibility_marks_and_uses_provider_gate(
    base_state, fake_device
) -> None:
    ...
    model = FakeModelClient(
        FakeModelResponse(
            "", '{"type":"intent","action":"tap","target_mark_id":"ax_1"}'
        )
    )
    ...
    assert (
        provider_summary["metadata"]["fallback_chain"][0]["provider"]
        == "accessibility_tree"
    )


def test_plan_node_includes_object_registry_sidecars_and_prompt(
    base_state, fake_device
) -> None:
    ...
    model = FakeModelClient(
        FakeModelResponse(
            "", '{"type":"intent","action":"tap","object_role":"video","ordinal":1}'
        )
    )
...
def test_plan_node_hybrid_filters_unrelated_accessibility_base_path(
    base_state, fake_device
) -> None:
...
def test_expected_outcome_and_verifier_evidence_are_regex_redacted(
    base_state, fake_device, tmp_path
) -> None:
    from phone_agent.graph.trace import JsonlTraceWriter

    writer = JsonlTraceWriter(
        trace_id="verifier-redaction", trace_dir=tmp_path, redact=False
    )
...
def test_plan_trace_parse_metadata_matches_legacy_dsl_parse_failure(
    ...
    writer = JsonlTraceWriter(
        trace_id="parse-fail-meta", trace_dir=tmp_path, redact=False
    )
    model = FakeModelClient(
        FakeModelResponse(
            "",
    ...
    assert (
        plan_result["payload"]["parse_metadata"]["parse_error_code"] == "invalid_json"
    )
...
def test_parse_reflection_action_structured_json_only() -> None:
    ...
    retry = parse_reflection_action(
        '{"verdict":"bad","failure_cause":"bad","suggested_strategy":"bad"}'
    )
...
def test_reflect_node_hard_verifier_failure_overrides_model_success(
    base_state, fake_device
) -> None:
    base_state["action_result"] = {
        "success": False,
        "should_finish": False,
        "message": "Action failed: boom",
    }
    model = FakeModelClient(
        FakeModelResponse(
            "ok",
...
def test_reflect_node_screen_change_is_only_weak_signal(
    base_state, fake_device
) -> None:
...
def test_reflect_node_type_text_postcondition_verifier_signal_is_advisory(
    ...
    base_state["observation"] = {
        "nodes": [{"role": "EditText", "text_summary": "before-only"}]
    }
    model = FakeModelClient(
        FakeModelResponse(
            "ok",
...
def test_reflect_node_hash_matches_text_segment(base_state, fake_device) -> None:
    ...
    assert result["verifier_status"] == "success"
    assert result["verifier_evidence"]["matched_postconditions"] == [
        "sha256:44ce7ae909bb"
    ]
...
def test_reflect_node_selected_object_text_matches_detail_page(
    ...
    model = FakeModelClient(
        FakeModelResponse(
            "ok",
            '{"verdict":"failed","failure_cause":"wrong_page","suggested_strategy":"retry","message":"model missed"}',
        )
    )
...
def test_reflect_node_selected_object_detects_still_on_feed(
    base_state, fake_device
) -> None:
    base_state["action_parsed"] = {
        "_metadata": "do",
...
def test_reflect_node_selected_object_feed_terms_win_over_generic_player_terms(
    base_state, fake_device
) -> None:
    base_state["action_parsed"] = {
        "_metadata": "do",
...
def test_reflect_node_input_focused_requires_focus_or_keyboard_signal(
    base_state, fake_device
) -> None:
    base_state["action_parsed"] = {
        "_metadata": "do",
    ...
    model = FakeModelClient(
        FakeModelResponse(
            "ok",
            '{"verdict":"partial","failure_cause":"unknown","suggested_strategy":"retry","message":"not enough"}',
        )
    )
...
def test_reflect_node_search_page_progress_prevents_takeover(
    base_state, fake_device
) -> None:
...
def test_reflect_node_vlm_success_with_missing_postconditions_does_not_succeed(
    base_state, fake_device
) -> None:
    base_state["action_parsed"] = {
        "_metadata": "do",
...
def test_reflect_node_does_not_use_stale_before_observation_for_postcondition(
    base_state, fake_device
) -> None:
    base_state["action_parsed"] = {"_metadata": "do", "action": "Type", "text": "hello"}
    base_state["expected_outcome"] = {
    ...
    base_state["observation"] = {
        "nodes": [{"role": "EditText", "text_summary": "hello"}]
    }
    model = FakeModelClient(
        FakeModelResponse(
            "ok",
...
def test_reflect_node_metadata_only_mark_id_does_not_satisfy_postcondition(
    base_state, fake_device
) -> None:
    base_state["action_parsed"] = {
        "_metadata": "do",
...
def test_reflect_node_does_not_assume_success_when_verifier_unknown_and_model_fails(
    base_state, fake_device
) -> None:
...
def test_reflect_node_hard_verifier_failure_blocks_finish_from_model(
    base_state, fake_device
) -> None:
    base_state["action_result"] = {
        "success": False,
...
def test_reflect_prompt_sanitizes_action_and_result_text(
    base_state, fake_device
) -> None:
...
def test_reflect_prompt_includes_before_after_observation_summaries(
    ...
    model = FakeModelClient(
        FakeModelResponse(
            "ok",
            '{"verdict":"failed","failure_cause":"wrong_page","suggested_strategy":"retry","message":"still home"}',
        )
    )
...
def test_plan_node_can_trace_unredacted_prompt_debug(
    ...
    prompt_debug = next(
        item for item in records if item["event"] == "plan_prompt_debug"
    )
...
def test_plan_node_inject_mode_redacts_sensitive_context(
    base_state, fake_device
) -> None:
    base_state["context_mode"] = "inject"
...
def test_plan_node_request_compaction_strips_historical_images_only(
    ...
    assert all(
        ...
        or all(item.get("type") != "image_url" for item in message["content"])
...
def test_step_zero_injects_app_registry_into_system_prompt(
    ...
    assert (
        "可用应用" in system_msg["content"] or "Available Apps" in system_msg["content"]
    )
...
def test_plan_node_injects_task_goal_after_message_compaction(
    base_state, fake_device
) -> None:
    from phone_agent.graph.goal_compiler import HeuristicGoalCompiler
    ...
    base_state["goal_contract"] = HeuristicGoalCompiler().compile(
...
def test_acceptance_node_accepts_pending_finish_with_final_goal_evidence(
    ...
    model = FakeModelClient(
        FakeModelResponse(
            "unused",
            '{"verdict":"succeeded","failure_cause":"none","suggested_strategy":"finish","message":"done","named_evidence":[{"criterion":"player_visible","screen_reference":"mark_id=player"}]}',
        )
    )
...
def test_expected_outcome_selected_video_object_defaults_to_page_opened(
    base_state, fake_device
) -> None:
...
def test_acceptance_node_generic_pending_finish_can_use_model_evidence(
    ...
    assert result["finish_validation_status"] == "success"
    assert (
        "task_done" in result["finish_validation_evidence"]["matched_terminal_evidence"]
    )
    assert result["goal_evidence_ledger"][-1]["criterion_id"] == "task_done"
...
def test_acceptance_node_rejects_ranked_finish_without_expected_rank_match(
    ...
    assert (
        "selected_rank_2"
        in result["finish_validation_evidence"]["missing_terminal_evidence"]
    )


@pytest.mark.parametrize(
    ("observed", "expected_status", "should_finish"),
    [("Singapore", "contradicted", False), ("Silverstone", "matched", True)],
)
def test_private_goal_value_survives_runtime_context_and_folds_semantics(
    base_state, fake_device, observed: str, expected_status: str, should_finish: bool
) -> None:
    from phone_agent.graph.goal import GoalContract, SuccessCriterion
    ...
    goal_result = goal_node(
        ...
        {"configurable": shared_config},
    )
    projected = json.dumps(goal_result, ensure_ascii=False)
    assert "Silverstone" not in projected
    assert requirements.task_hash not in projected
    assert all(value not in projected for value in requirements.target_entity_hashes)

    base_state.update(goal_result)
    base_state["task"] = task
    base_state["action_parsed"] = {
        "_metadata": "finish",
        "message": "done",
        "matched_terminal_evidence": ["topic"],
    }
    base_state["action_result"] = {
        "success": True,
        "should_finish": False,
    ...
    model = FakeModelClient(
        FakeModelResponse(
            ...
            '"suggested_strategy":"finish","message":"done",'
            '"named_evidence":[{"criterion":"topic",'
            f'"screen_reference":"region-1","observed_value":"{observed}",'
        ...
        base_state,
        {
            "configurable": {
                **shared_config,
                "model_client": model,
                "device_factory": fake_device,
                "visual_fact_extractor": lambda _context, _predicate: (
                    {
                        "value": observed,
    ...
    assert result["finished"] is should_finish
    assert result["failure_cause"] is (None if should_finish else "goal_not_satisfied")
    assert any(
        item.get("criterion_id") == "topic" and item.get("status") == expected_status
        for item in result["goal_evidence_ledger"]
    )
    assert "Silverstone" not in json.dumps(

...
def test_remote_provider_metadata_summary_is_allowlisted() -> None:
    ...
    class Provider:
        ...
        def provide_marks(self, screenshot, screen_binding, hints=None, timeout=None):
            mark = MarkCandidate(mark_id="remote_1_1", bbox=[100, 200, 300, 400], center=[200, 300], source="remote_openai")
            return MarkProviderResult(
                success=True,
                provider="remote_openai;13800138000",
                screen_id=screen_binding.screen_id,
                raw_screenshot_hash=screen_binding.raw_screenshot_hash,
                provider_input_hash="hash",
                marks=[mark],
                ...
                metadata={
                    ...
                    "base_url_host": "api.stepfun.com",
                    "request_image_width": 100,
                    "request_image_height": 200,
                    "request_image_length": 1234,
                    "hint_length": 12,
                    "raw_hint_sent": False,
                    "raw_response": "13800138000",
                    "api_key": "sk-secret",
                    "fallback_chain": [{"provider": "evil;13800138000", "failure_code": "bad;secret"}],
                },
            )

    observation = build_observation(
        screenshot=SimpleNamespace(base64_data="fake-image", width=1000, height=2000),
        current_app="Settings",
        mark_providers=[Provider()],
        provider_hints=[],
    )
    summary = observation.mark_provider_observation["providers"][0]
    raw = json.dumps(summary, ensure_ascii=False)

    assert summary["provider"] == "unknown"
    assert summary["metadata"]["remote"]["model"] == "step-3.7-flash"
    assert summary["metadata"]["remote"]["base_url_host"] == "api.stepfun.com"
    assert summary["metadata"]["fallback_chain"][0]["provider"] == "unknown"
    assert "13800138000" not in raw

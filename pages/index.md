...
- **App-KB 自积累记忆** — 同步本机应用、隐式纠正沉淀别名、按设备隔离，越用越准
- **经验档案 + 语义回想** — 每次 run 落结构化档案（隐私白名单）；本地向量索引 shadow 回想，自验证命中率，达标前不注入
- **能力注册表** — 每个能力可关/可版本化/可回滚；依赖缺失可见待岗；每 run 能力快照可审计
- **长任务可控** — token 预算限制成本；两级 auto-compact 在接近上下文窗口时保留关键状态
- **实时控制台** — 时间线步骤、点步骤钉截图、记忆页、软停止

...
class MainViewModelTest {
    ...
    fun `思考消息应该包含正确的内容`() = runTest {
        ...
        val state = viewModel.state.value
        assertEquals(1, state.messages.size)
        val message = state.messages[0] as com.example.autoglm.data.MessageItem.ThinkMessage
    ...
    fun `带步骤前缀的思考消息应该正确显示`() = runTest {
        ...
        val msg1 = state.messages[0] as com.example.autoglm.data.MessageItem.ThinkMessage
        assertTrue(msg1.content.startsWith("[1]"))

        val msg2 = state.messages[1] as com.example.autoglm.data.MessageItem.ThinkMessage
        assertTrue(msg2.content.startsWith("[2]"))

        val msg3 = state.messages[2] as com.example.autoglm.data.MessageItem.ThinkMessage
    ...
    fun `操作消息应该包含正确的Action对象`() = runTest {
        // Given
        val action = com.example.autoglm.data.Action(
            action = "tap",
        ...
        val state = viewModel.state.value
        assertEquals(1, state.messages.size)
        val message = state.messages[0] as com.example.autoglm.data.MessageItem.ActionMessage
    ...
    fun `混合消息类型应该按顺序添加`() = runTest {
        ...
        assertTrue(state.messages[0] is com.example.autoglm.data.MessageItem.ThinkMessage)
        assertTrue(state.messages[1] is com.example.autoglm.data.MessageItem.ActionMessage)
        assertTrue(state.messages[2] is com.example.autoglm.data.MessageItem.SystemMessage)
        assertTrue(state.messages[3] is com.example.autoglm.data.MessageItem.ThinkMessage)
    }
    ...
    fun `任务执行期间添加的消息应该保留在ViewModel中`() = runTest {
        ...
        viewModel.addActionMessage(
            com.example.autoglm.data.Action(
                action = "tap",
                location = listOf(100, 200)
            )
        )
    ...
    fun `任务失败时的错误消息应该被记录`() = runTest {
        ...
        val state = viewModel.state.value
        assertEquals(1, state.messages.size)
        val message = state.messages[0] as com.example.autoglm.data.MessageItem.SystemMessage
    ...
    fun `PreCheckResult成功时success应该为true`() {
        // When
        val result = com.example.autoglm.ui.viewmodel.PreCheckResult(success = true)
    ...
    fun `PreCheckResult失败时应该包含错误消息`() {
        // When
        val errorMsg = "网络连接失败"
        val result = com.example.autoglm.ui.viewmodel.PreCheckResult(success = false, errorMessage = errorMsg)

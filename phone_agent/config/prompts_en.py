...
- Locate (internal tool): `{"type":"intent","action":"locate","target_text_hint":"focused short description of a visible element"}`. Pass a visual description of the target and receive an executable mark (registered into Screen marks, usable on the next step). Cost: ~2s latency; max 3 per run (see the budget block "locate x/3 left"); the same description on the same screen is rejected when repeated. Failure semantics: zero or multiple boxes fail and produce no mark; the failure reason appears in the previous step's result. `target_text_hint` must be a focused short description of a visible element (≤64 characters recommended). Never paste the whole task sentence, and never include private raw text (phone numbers, emails, order ids, captcha codes). Locate never executes a tap itself. You may consider it when the current Screen marks do not cover your target. Optional `scope_mark_id`: pass an existing Screen mark that contains the target (for example a container block) to narrow the search to that mark's region — this improves accuracy for small or dense targets. The scope must reference a mark that exists on the current screen; cropping only affects the detection region, and the returned mark is still in full-screen coordinates.

...
- Locate（内部工具）：`{"type":"intent","action":"locate","target_text_hint":"可见元素的聚焦短描述","scope_mark_id":"ax_5"}`。用法是**先指区域，再指目标**：必须同时传入 scope（二选一）——形态A：`scope_mark_id`（一个包含目标的已有 Screen mark，如容器区块）；形态B：`scope_start_mark_id`+`scope_end_mark_id`（两个锚点 mark 夹出的区间，检测区域为 [起点.top, 终点.top) 的横向条带；只给 start 时到 start 所在容器底部）。scope 决定 LocateAnything 的搜索范围：它只在该区域内搜索（该区域会被从截图裁出单独检测），目标不在区域内必然失败；区域必须在画面上【空间包含】目标本身——空间包含≠语义相关，文字标签/标题不是容器（如"2026年10月"标题里没有任何日期格子）。搜索区域越紧，定位准确度越高；拿不准时选更大的容器（最大≈全屏，合法）。当目标位于两个文字锚点之间时，用 start/end 夹出区间即可圈出目标所在块——例如日历中目标日期在"X月"标题与下一月标题之间，用两个月份标题做区间即可圈出整个月块，无需知道目标在第几行。区域选错导致 0 框/多框失败时，可调整/扩大 scope 区域后重试。传入目标的视觉描述，返回一个可执行 mark（注册进 Screen marks，下一步即可用）。成本：约 2s 延迟；同一屏幕同一描述重复调用会被拒绝。`target_text_hint` 只写可见元素的聚焦短描述（建议 ≤64 字符），禁止整句任务、禁止隐私原文（手机号/邮箱/订单号/验证码等）；locate 不会直接执行点击。当 Screen marks 不能覆盖你的目标时可以使用它。scope 必须引用当前屏幕存在的 mark；裁剪只影响检测范围，返回的 mark 仍是全屏坐标。
